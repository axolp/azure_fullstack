...
function Episode2() {
    ...
    const transcryptDivRef= useRef(null);
  ...
  const TranscryptCharacterDiv= (hanzi, pinyin, definicja) =>{
    //console.log("tworze diva")
    const TranscryptContainer= document.createElement('div');
    TranscryptContainer.classList.add('CharacterContainer');
    ...
    TranscryptContainer.addEventListener('click', () => {
      //console.log(CharacterContainer.innerText);
      //console.log(CharacterContainer.innerText.replace(/\n/g, '\\n'));

      //console.log('Kliknięto kontener znaku');
      setFContent(TranscryptContainer.innerText);
      
  });

    

  TranscryptContainer.appendChild(AngContainer);
  TranscryptContainer.appendChild(HanZiContainer);
  TranscryptContainer.appendChild(PinYinContainer);

     transcryptDivRef.current.appendChild(TranscryptContainer);
  ...
  const DeleteCharacterDivs=()=> {
    ...
    return (
        <div className='videoPlayer'>
            ...
            <div ref= {lyricsDivRef} className='neo'></div>
            <div ref= {transcryptDivRef} className='transcrypt'></div>
        </div>
    );
}
